...
BENCHMARK_NAME = "efficientnetb0_stanfordcars"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="EfficientNet-B0 + Stanford Cars WD-method benchmark.")
...
def main() -> None:
    ...
    data_root, train_set, val_set, num_classes, downloaded = build_stanfordcars_datasets(
        args.data_root,
        tf_train,
        tf_val,
    ...
    print("Stanford Cars auto-downloaded:", downloaded)
    ...
    print(
        ...
        f"val_batches={max_val_batches} | wd={args.wd:g} | workers={args.num_workers}"
    )
    ...
    try:
        for epoch in range(1, epochs + 1):
            ...
            for x, y in pbar:
                ...
                global_step += 1
                if args.method == "alphadecay":
                    alpha_row = maybe_update_alphadecay(
                        method_state=method_state,
                        untuned_wd=args.wd,
                        update_step=global_step,
                    )
                    if alpha_row is not None:
                        method_state.wd_history.append(alpha_row)
                else:
                    wd_values = maybe_update_ouidecay(
                        method_state=method_state,
                        update_step=global_step,
                    )
    ...
    payload = build_run_payload(
        kind="stanfordcars_classification_run",
